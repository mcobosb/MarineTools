...
def openstreetImages(lon, lat, distx, disty, sitename, style:str="satellite"):
    ...
    osm_image(lon, lat, sitename=sitename,
        style=style, distx=distx, disty=disty)
...
def calc_extent(lon,lat,distx, disty):
    ...
    top_left = cgeo.Geodesic().direct(points=(lon,lat),azimuths=-(90-angle),distances=dist_cnr)[:,0:2][0]
    bot_right = cgeo.Geodesic().direct(points=(lon,lat),azimuths=90+angle,distances=dist_cnr)[:,0:2][0]
